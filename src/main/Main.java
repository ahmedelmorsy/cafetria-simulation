...
public class Main {    
    ...
    public static void main(String[] args) {
        FileLogger result = new FileLogger(new File("results.data"));
        for (int i = 0; i < 15; i++) {
            Statistics.reset();
            ...
            result.log(
                    ...
                    + "," + (0.8 * Statistics.getMaxDelayForCust(Const.CUST_HOTFOOD)
                    + 0.15 * Statistics.getMaxDelayForCust(Const.CUST_SANDWICHES) 
                    + 0.05 * Statistics.getMaxDelayForCust(Const.CUST_DRINKS))//20
